<template>
    <div>
        ...
        >
            ...
            >
                <template #default="record">
                    <nuxt-link :to="`/news/${record.id}`" class="hover:underline">
                        {{ record.title }}
                    </nuxt-link>
                </template>
            </a-table-column>
                ...
                <template #default="record">
                    <a-dropdown>
                        ...
                        <a-menu slot="overlay">
                            ...
                            <a-menu-item class="!py-2" @click="$refs.newsDeleteDialog.open(record)">
        ...
        <NewsDeleteDialog ref="newsDeleteDialog" />
    </div>
</template>

<script>
    import { mapDataFromOptions } from '@/utils/data';
    import { NEWS_STATUS, NEWS_STATUS_OPTIONS } from '@/constants/news/status';
    import NewsDeleteDialog from '@/components/news/dialogs/Delete.vue';
    ...
    export default {
        ...
        methods: {
            ...
            async active(data) {
                try {
                    await this.$api.news.setActive(data.id);
            ...
            async inactive(data) {
                try {
                    await this.$api.news.setInactive(data.id);
                    await this.$store.dispatch('news/fetchAll', this.$route.query);
